...
class STIPImgDataset(data.Dataset):
  def __init__(self):
    ...
    self.sample_interval = 200     # Interval at which a new set of frames is sampled
  ...
  def generate_frame_correspondences(self, fps, frame_count):
    ...
    ret = []
    step = 20//fps
    for video in sorted(os.listdir(self.data_path))[:18]: #:18
      ...
      for num, frame in enumerate(l):
        ...
        if num + 500 <= llen:
           ...
           os.path.exists(self.right_img_path.format(video) + self.img_path.format(f_number + offset[1])):
            ...
            print("Invalid frame set found:", f_number + offset[0], f_number, f_number + offset[1])
