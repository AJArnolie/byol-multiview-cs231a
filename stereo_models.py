...
class BYOLMultiView(nn.Module):
    ...
    def forward(self, s, m1, m3):
        return self.single(s), self.multi1(m1), self.multi3(m3)

    def forward_eval(self, s):
...
class Loss(nn.Module):
    ...
    def forward(self, rep_s, rep_m1, rep_m3):     
        ...
        L_s_m1 = self.loss(F.normalize(pred_s), F.normalize(proj_m1.detach()))
        L_m1_s = self.loss(F.normalize(pred_m1), F.normalize(proj_s.detach()))
        
        L_s_m3 = self.loss(F.normalize(pred_s), F.normalize(proj_m3.detach()))
        L_m3_s = self.loss(F.normalize(pred_m3), F.normalize(proj_s.detach()))
        ...
        return L_s_m1 + L_m1_s + L_s_m3 + L_m3_s
